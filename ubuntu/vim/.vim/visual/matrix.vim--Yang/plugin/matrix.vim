...
" x: column, y: line
" b:x = random column 
" s: random delay speed [s:mindelay, s:maxdelay]
" t: timer [0, s:maxdelay - 1] show char from s
" Each column has the same timer.
" head:
" len: random height
" reserve:
" i = [0, (b:w - 1)/2 - 3] = b:objcount
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
function! s:DrawObject(i)
   "write two char, one is character from b:d, the other one is control char
   "( _:head, `:bold dark green char ) 
   let x = b:x{a:i} * 2 + 1
   let y = b:y{a:i}
   ...
   if 1 <= y && y <= b:h
      ...
      "echo '5norm! :'. y . nr2char(13) . x . '|R  ' . nr2char(27)
   endif
...
function! s:Animate()
   ...
   while i < b:objcount
      " Animate object
      "echo 't:' . b:t{i} . ' y:' . b:y{i} . ' len:' . b:len{i} . ' s:' . b:s{b:x{i}} . ' x:' . b:x{i}
      "column timer : time's up
      if b:t{i} <= 0
         " b:len = [3, b:h + 2] => b:y{i} - b:len{i} = [-b:h + 2, -2]
         if b:y{i} - b:len{i} <= b:h
            ...
            " Regenerate
            call s:CreateObject(i)  "reset timer
            "echo '>t:' . b:t{i} . ' y:' . b:y{i} . ' len:' . b:len{i} . ' s:' . b:s{b:x{i}} . ' x:' . b:x{i}
         endif
      endif
      "countdown
...
function! s:Init()
   ...
   let s:o_so = &so
   let s:o_ve = &ve
   " ch: cmdheight. Number of screen lines to use for the command-line.
   " ls: laststatus. The value of this option influences when the last window will have a
   "     status line.
   " lz: lazyredraw.
   " nosm: no showmatch.
   " siso: sidescrolloff.
